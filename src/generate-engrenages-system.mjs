...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'engrenages', 'Engrenages', and 'ENGRENAGES'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.engrenages/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.engrenages/g),
   ...
   * Rename files.
   *
   * Rename files that had engrenages in their name, such as
   * css/engrenages.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*engrenages*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('engrenages', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-engrenages-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob Engrenages's files so that we can process them.
    glob('*', {ignore: 'node_modules/**' }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace engrenages name mentions in files.
      generator.replaceFileContents();
      // Rename files that had engrenages in their name.
